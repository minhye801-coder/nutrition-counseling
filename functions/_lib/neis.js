...
export async function searchSchools(request, env) {
  ...
  if (schoolKind) params.SCHUL_KND_SC_NM = schoolKind;
...
export async function getSchoolMeals(request, env) {
  const url = new URL(request.url);
...
async function fetchNeisRows(
  ...
  if (!apiKey) {
    throw new Error(
      "NEIS_API_KEY가 등록되지 않았습니다. Cloudflare Pages 프로젝트의 Settings → Variables and Secrets에서 Secret으로 등록하세요.",
    );
  }
  ...
  /*
   * 인증키가 포함된 실제 주소를 캐시 키로 쓰지 않기 위해
   * 키를 제외한 내부 캐시 주소를 만듭니다.
   */
  const safeCacheUrl = new URL(
    `https://nutrition-counseling-cache.internal/${endpoint}`,
  );
  ...
  const response = await fetch(upstreamUrl.toString(), {
    ...
    headers: {
      ...
      "User-Agent": "NutritionCounselingTasteMind/2.1",
    },
  });
