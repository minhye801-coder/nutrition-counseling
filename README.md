...
- 회기마다 정식 커리큘럼 단계명 부여 (1단계 관찰과 표현 → 2단계 감각과 어려움 → 3단계 전략과 미션 → 4단계 실천과 도움 → 5단계 성장과 안내)
- 처음 접속 시(또는 언제든 다시 보기) 프로그램을 소개하는 온보딩 화면
- 최근 회기와 오늘의 한마디를 한 장으로 요약하는 상담 리포트 (인쇄/저장 가능)
- 5회기를 모두 마치면 발급되는 수료증 (인쇄/저장 가능)
- 미션 돌아보기(점검 및 한 줄 회고), 최근 점검 기록
- 오늘의 한마디: 회기 테마에 맞는 영양상담 질문 카드를 타로카드처럼 뒤집어 보며 학생과 대화하는 활동 (응답 기록은 선택사항)
- 나의 감자: 완료한 회기 수에 따라 점점 씩씩해지고 빛나는 감자 캐릭터를 메인 화면에 표시
...
- 자유게임 6종: 퀴즈 4종(음식 감각 탐정 / 식판 친구 찾기 / 생활리듬 퀘스트 / 음식 안전 신호, 각 3문항, 타임어택 모드 선택 가능) + 아케이드 2종(블록깨기, 구슬 튕겨서 넣기)
- 탐험노트: 자동 생성되는 "나의 음식생활 설명서", 자신감 변화 그래프, 회기별 기록 열람(최신순/시간순)
- 체험 데이터 안내 화면, "기록 초기화"와 "별명 완전 삭제" 기능
